...
class PortalCategoryModel extends Model
{
     ...
     * @return string
     */
    public function adminCategoryTree($selectId = 0, $currentCid = 0,$type=1)
    {
        $where = ['delete_time' => 0,'type'=>$type];
        if (!empty($currentCid)) {
     ...
     * @return string
     */
    public function adminCategoryTableTree($currentIds = 0, $tpl = '',$type=1)
    {
        $where = ['delete_time' => 0,'type'=>$type];
//        if (!empty($currentCid)) {
//            $where['id'] = ['neq', $currentCid];
//        }
        ...
        foreach ($categories as $item) {
            ...
            $controller = $item['type']==1?'AdminCategory':'PrizeCategory';
            $item['str_action'] = '<a href="' . url("$controller/add", ["parent" => $item['id']]) . '">添加子分类</a>  <a href="' . url("$controller/edit", ["id" => $item['id']]) . '">' . lang('EDIT') . '</a>  <a class="js-ajax-delete" href="' . url("$controller/delete", ["id" => $item['id']]) . '">' . lang('DELETE') . '</a> ';
            array_push($newCategories, $item);
        }
