 ...
 * )
 */
class TurntableRulesController extends AdminBaseController
{
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            $result = $this->validate($this->request->param(), 'app\turntable\validate\TurntableRulesValidate');
            var_dump($_POST);exit;
            $result = true;
            if ($result !== true) {
                $this->error($result);
            } else {
                $result             = DB::name('turntable_rules')->insertGetId($_POST);
                if ($result !== false) {
                    $this->success("添加成功！", url("TurnTableRules/index"));
                } else {
                    $this->error("添加失败！");
                }
            }
